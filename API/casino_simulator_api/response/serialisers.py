...
'''
    This file serialises the data for the API request;

    GameSessionSerialiser: Serialises the GameSession model with the following fields:
        - session_id: The unique identifier for the game session.
        - created_at: The timestamp when the session was created.
        - game_state: The current state of the game.

    AIResponseSerialiser: Serialises the AIResponse model with the following fields:
    ...
    ApiManager in Unity expects to function with this exact structure to deserialise the 
    response into GameState objects with player_name, score, level, and status properties. Without this
    precise format, the game would fail to parse responses, as evidenced by how the HandleApiResponse
    method processes JSON payloads to extract both the AI text and game state in a single transaction.
'''
